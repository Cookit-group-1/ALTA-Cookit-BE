...
func initTransactionRouter(db *gorm.DB, e *echo.Echo) {
	...
	e.PUT(fmt.Sprintf("/users/transactions/:%s/status", consts.ECHO_P_TransactionId), handler.UpdateTransactionStatusById, middlewares.JWTMiddleware())
	e.POST("/users/transactions/status", handler.UpdateTransactionStatusByMidtrans)
}
